...
class Quizes {
    ...
    touchX  = [];
    answer=(btn)=>{
        this.id = Number(btn.getAttribute("id").slice(3));
        if(!this.answered){
            this.answered = true;
            if(this.arr[this.pointer].site === this.choices[this.id]){
                this.blue = this.id;
                elem("quiz_answer").innerText = `〇${this.arr[this.pointer].site}`
                elem(`btn${this.id}`).className = "choices back_green";
                elem("quiz_answer").className = "font_green";
                this.exclude.push(this.arr[this.pointer]);
                deleteSample();
                createSample();
            } else {
                elem("quiz_answer").innerText = `✕${this.arr[this.pointer].site}`;
                elem(`btn${this.id}`).className = "choices back_red";
                this.blue = this.choices.indexOf(this.arr[this.pointer].site);
                this.red = this.id;
                elem(`btn${this.blue}`).className = "font_red";
                elem("quiz_answer").className = "font_red";
            }
            elem("quiz_counter").innerText = `Your Score: ${this.exclude.length}/${this.counter}`;
        } else {
            if (this.id === this.blue) this.next();
        }
    };
    next=()=>{
        this.counter++;
        this.answered = false;
        elem(`btn${this.red}`).className = "choices";
        elem(`btn${this.blue}`).className = "choices";
        this.pointer++;
    };
