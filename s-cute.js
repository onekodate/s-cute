...
function setting2summary(){
    ...
    if(se.s_d_g==="Scatter"){
        ...
        const sorted=Array.from(new Set(arr.map(val=>Number(val.num)))).sort((a,b)=>a-b);
...
function search(input){
    ...
    const str1="<table><tr><th>#</th><th>Girl</th><th>Date</th><th>Genre</th><th>Title</th><th>Site</th></tr>";
    const str2=arr.sort((a,b)=>b.num-a.num).map(val=>{
        return ["<tr><td>",val.num,
                "</td><td>",val.girl,
                "</td><td>",val.date,
                "</td><td nowrap>",val.genre,
                "</td><td class=break><a target='_blank' href='",val.url,
                "'>",val.title,
                "</a></td><td class=break>",val.site,
                "</td></tr>"].join("");
    }).join("");
...
function next_quiz(){
    quiz.answered=false;
    elem("btn"+quiz.id).className="choices";
...
function clear_storage(boolean){
    let t=boolean;
    if(!boolean) t=window.alert("May I clear storage of your answer data?");
    if(t){
        deleteSample();
        quiz.exclude=[];
        createSample();
    ...
    openRequest.onsuccess=(e)=>{
        ...
        if(db.objectStoreNames.length>0){
            ...
            getRequest.onsuccess=(e)=>{
                quiz.exclude=getRequest.result.result;
                if(quiz.exclude){

                }else console.log("Not Found");
            }
            getRequest.onerror=(e)=>console.log(e.message);
        }else{
            console.log("db none");
            db.close();
            deleteSample();
